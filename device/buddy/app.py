...
畳んで生きた REPL の上へ返る (`finally` の `to_repl` の枝)。意図した出口は
...
def run():
    ...
    try:
        while True:
            ...
            # 溜めておいた UI の仕事をここで捌く。LCD への書き込みが、周期的
            # な footer の描画や protocol のイベントが起こす prompt の描画と
            # 交錯しないようにするため。とくに set_connection は header の帯を
            # まるごと描き直し、これは SPI のトランザクション何本ぶんもある。
            ...
            if router.chat_dirty:
                ...
                if chat.active:
                    ...
                    # chat.clear が panel を返してきた。chat は header まで
                    # 覆っているので、dashboard には main panel だけでなく
                    # chrome の全面描き直しが要る。_redraw_chrome はまさに
                    # そのための upstream 自身の private なヘルパで、将来の
                    # bundle で名前が変わったら、header 以外を全部カバーする
                    # public な呼び出しへ落ちる。
            ...
            if time.ticks_diff(now, last_footer_ms) >= footer_interval:
                state.tick_nap()
                # stats の footer は y=96..110 に描く。そこは chat の領域の
                # 内側で、これを飛ばすことが transcript を 3 秒ごとに打ち抜か
                # ないでいる理由になっている。SPI のトランザクション何本ぶんも
                # あり、再生中の tick には余裕が無い — underrun は耳に付くが、
                # 3 秒古いバッテリ表示は付かない。
        ...
        # 畳む順は buddy_app.py に倣う: まずトランスポート、次に画面を黒で
        # 塗り潰し、それから制御を返す。トランスポートを落とす前に stop():
        # あれはトランスポート越しの転送を切り、キューに 1 秒ぶんの音声を
        # 抱えたままのスピーカーを黙らせる唯一のもの。
        try:
            speech.stop()
        except Exception as e:
