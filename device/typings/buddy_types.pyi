...
class Socket(Protocol):
    """accept が返す 1 本の接続のうち、`buddy.netlink` が触るぶん。"""
...
class Listener(Socket, Protocol):
    """listen する側。`socket.socket()` が返すもの。"""
...
class SocketModule(Protocol):
    """`socket` モジュールの面。`buddy.netlink.BuddyNet` が注入で受け取る。"""

    # 読むだけなので property。本物のモジュール属性も、テストの fake の
    # クラス属性も、これなら通る。
    @property
    def SOL_SOCKET(self) -> int: ...
    @property
    def SO_REUSEADDR(self) -> int: ...
...
class Transport(AckSink, Protocol):
    """`buddy.serial.BuddySerial` / `buddy.netlink.BuddyNet` のうち、アプリと
    `buddy.mux.TransportMux` が触るぶん。"""
...
class DebugModule(Protocol):
    ...
    # `Chat` / `Speech` と同じ面。1 行を解くのは `buddy/router.py` の
    # `_decode()` だけで、ここへ来るのは解けた後の dict。
    def handle(self, msg: dict[str, object], /) -> dict[str, object] | None: ...
