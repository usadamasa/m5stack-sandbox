...
**何をやっているセッションなのかは transcript から読む。** hook が線に載せられるのは
`kind` と 100 文字の `detail` だけで、それは `tool: Bash: uv run pytest` のような行に
しかならない。そこで hook は `session_id` も載せ、daemon が
`~/.claude/projects/*/<session_id>.jsonl` を自分で引いて、セッションの題 (`ai-title`)、
最後の指示 (`last-prompt`)、cwd とブランチを拾う。3 つ動いていれば 3 つとも渡るので、
デバイスは別の窓で進んでいる仕事にも触れられる。

`transcript_path` は hook の payload にあるが、**送らない**。socket はこのマシンの
誰にでも開いているので、受け取ったパスを daemon が開けば、送り主が読ませたいどんな
ファイルの断片でもプロンプトへ貼られ、デバイスが読み上げることになる。線に載るのは
UUID として検められる `session_id` だけで、パスは daemon が組み立てる。会話の本文も
読まない — 拾ったものはパネルに出て読み上げられるので、題と直前の指示までに留める。
`BUDDY_CHATTER_SESSIONS=0` で切れる。
